...
const Settings: React.FC<SettingsProps> = ({ currentUser, currentTrip, updateUser, updateTrip }) => {
  ...
  const isOwner = currentUser.id === currentTrip.ownerId;
      ...
      <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        ...
        {!isOwner && (
             <div className="p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
                Only the trip owner can modify the trip's budget and dates.
            </div>
        )}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" id="start-date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm disabled:opacity-70"/>
                        </div>
                        <div>
                            <label htmlFor="end-date" className="block text-sm font-medium text-gray-700 dark:text-gray-300">End Date</label>
                            <input type="date" id="end-date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm disabled:opacity-70"/>
                        </div>
                    </div>
                </div>
                        ...
                        {Object.values(Category).map(cat => (
                            <div key={cat}>
                                ...
                                <div className="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    placeholder="0.00"
                                    className="block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500 sm:text-sm pl-7 disabled:opacity-70"
                                    />
                                </div>
                            </div>
                        ))}
